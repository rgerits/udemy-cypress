...
const createEsbuildConfig = (ctx, channels) => {
  ...
  let plugins = [deprecatedRemixPackage.deprecatedRemixPackagePlugin(ctx), cssModuleImports.cssModulesPlugin(ctx, {
    ...
    sideEffects: false
  }), ctx.config.future.unstable_dev ? routes_unstable.browserRouteModulesPlugin(ctx, routeModulePaths) : routes.browserRouteModulesPlugin(ctx, /\?browser$/), mdx.mdxPlugin(ctx), emptyModules.emptyModulesPlugin(ctx, /\.server(\.[jt]sx?)?$/), esbuildPluginPolyfillNode.polyfillNode(), external.externalPlugin(/^node:.*/, {
    sideEffects: false
  }), {
